...
SELECT
    ...
    o.example,
    o.option_from,
    COALESCE(GROUP_CONCAT(s.url), '') AS source_list
FROM
    options o
LEFT JOIN
    source_options so
...
LEFT JOIN
    sources s
    ON so.source_id = s.id
WHERE
    o.id IN (
        SELECT option_id FROM options_fts
...
options (
    option_name, description, option_type, option_from, default_value, example
)
VALUES (?, ?, ?, ?, ?, ?) RETURNING *;

-- name: AddSource :one
INSERT INTO sources (url) VALUES (?) ON CONFLICT (url) DO UPDATE SET
    url = excluded.url
RETURNING *;
